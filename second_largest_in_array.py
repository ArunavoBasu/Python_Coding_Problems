...
def getSecondLargest(arr: List) -> int :
    # Using set to store the unique values only
    # Converting again as list
    # Sorting that list
    unique_sorted_list = sorted(list(set(arr)))
    if len(unique_sorted_list) > 1:
        return unique_sorted_list[-2]
        ## As sorted gives ascending order list, so the position of second largest element will be -> "-2"
    else:
        return -1
